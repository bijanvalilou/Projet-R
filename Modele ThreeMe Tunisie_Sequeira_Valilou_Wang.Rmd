...
ggplot() + 
  ...
  labs(y = "GDP variation (%)", title = "GDP in volume. Variation in relation to the baseline") +
  scale_x_continuous(breaks=seq(2015,2050,5))
```
...
ggplot() + 
  ...
  labs(y = "Employment variation (%)", title = "Employment. Variation in relation to the baseline") +
  scale_x_continuous(breaks=seq(2015,2050,5))
```
...
ggplot() + 
  ...
  labs(y = "CO2 emissions variation (%)", title = "CO2 emissions. Variation in relation to the baseline") +
  scale_x_continuous(breaks=seq(2015,2050,5))
```
...
ggplot() + 
  ...
  labs(y = "Gross nominal wage (%)", title = "Gross nominal wage. Variation in relation to the baseline") +
  scale_x_continuous(breaks=seq(2015,2050,5))
...
Fig.1 <-
  ...
  geom_bar(stat = 'identity', position = 'stack') + guides(fill = guide_legend(reverse = TRUE)) + 
  labs(x = "year", y = "Share of energy consumption (%)", title = "Fig.x Energy subsidies scenario with redistribution") + 
...
plot1 <- ggplot() + 
 ...
 geom_line( aes(x = Sc4[,"year"],y = S4_Macro[,"PIB"], group = 1, color = "Energy subsidies removal with redistribution")) + 
  ...
  labs(x = "Year", y = " GDP (%)", title = "Fig. X GDP") + 
...
plot3 <- ggplot() + 
 ...
 geom_line( aes(x = Sc4[,"year"],y = S4_Macro[,"Taux.de.chômage"], group = 1, color = "Levée de \nsubventions")) + 
  ...
  labs(x = "Year", y = "Rate of unemployment (%)", title = "Fig. X  Rate of \nunemployment") + 
...
plot2 <- ggplot() + 
 ...
 geom_line( aes(x = Sc4[,"year"],y = S4_Macro[,"Investissement"], group = 1, color = "Levée de \nsubventions")) + 
  ...
  labs(x = "Year", y = "Investment (%)", title = "Fig. X Investment") + 
    theme(axis.title=element_text(size=9),axis.text=element_text(size=6), title = element_text(size=9), plot.title = element_text(hjust = 0.5, vjust = 0.5)) +
  scale_x_continuous(breaks=seq(2015,2050,5))
...
plot4 <- ggplot() + 
  ...
  labs(x = "Année", y = " Co2 (%)", title = "Fig.x CO2 Emissions (%)") + 
    theme(axis.title=element_text(size=9),axis.text=element_text(size=6),title = element_text(size=9), plot.title = element_text(hjust = 0.5, vjust = 0.5)) +
  scale_x_continuous(breaks=seq(2015,2050,5))
...
The national low carbon strategy creates winners and losers. The electricity sector is a major beneficiary of the policy mix, while the energy transformation sector experiences a sharp decline in its value added (see figure X).
...
Figure X shows that electricity increases from 19% of final energy consumption in Tunisia in 2050 to 30% in 2050.
...
Fig.1 <-
  ...
  geom_bar(stat = 'identity', position = 'stack') + guides(fill = guide_legend(reverse = TRUE)) + 
  labs(x = "year", y = "Share of energy consumption (%)", title = "Fig.x National Low Carbon strategy") + 
    theme(axis.title=element_text(size=7),title = element_text(size=9), plot.title = element_text(hjust = 0.5, vjust = 0.5)) +
  scale_x_continuous(breaks=seq(2015,2050,5))+
scale_y_continuous(labels = scales::percent_format(accuracy = 1))
...
When working with an Integrated Assessment Model as is the case of ThreeME we should pay attention not to attach too much importance to precise figures but instead to keep the whole in perspective, to focus on trends rather than on specific values. Another thing to keep in mind is that the model works on the basis of hypothesis and that this hypothesis influence in one way or another the

Amelioration du cadre statistique : pb du secteur informel non pris en compte par la comptabilité nationale
