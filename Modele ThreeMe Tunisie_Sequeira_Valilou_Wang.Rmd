...
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyfoot[CO,CE]{Model ThreeMe Tunisia}
...
In general, the presence of an environmental externality suggests that there is a marginal social cost that exceeds the marginal private cost. In the absence of regulation, the difference between these two costs (known as the marginal external cost of damage) is not covered by the polluter. Consider the particular case of the externalities associated with the release of CO2 in the atmosphere. The carbon tax is the tax that comes reflect that marginal external damage.
...
There are no general rules regarding the way in which revenues are recycled. Some authors give guidelines about which type of redistribution to choose depending on particular economic and sociopolitical circumstances. An example of that is the diagram tree below, oriented to help decision.
...
In addition to the carbon tax, price signals have been introduced in order to achieve energy consumption targets by source.
...
Last but not least, social and political acceptability of environmental policies are not taken into account in the model. In order to achieve the emission reduction targets, the conditions for the acceptability of the increase in the CO2 price must be ensured [\@combet] . Environmental policies are much more than an economic efficiency challenge or a fiscal instrument that the national administration can manage alone; they should take into account distributional fairness, political trust and policy stability [\@klenert2018a] . In this sense, the choice of revenue usage is crucial and should aim to enhance acceptability. There is no universal solution, instead the particularities of the Tunisian society and economy should be taken into account in order to carve out the policy that best suits them.
