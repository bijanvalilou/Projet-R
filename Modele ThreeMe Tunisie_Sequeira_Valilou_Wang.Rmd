...
ggplot() + 
  ...
  theme(axis.title=element_text(size=9), plot.title = element_text(hjust = 0.5, vjust = 0.5))
```
...
kable(Table_Macro_CT, booktabs = T, longtable = T, linesep = "",
      caption = "Macroecnomic indicators - Tax carbon with redistribution, \n in relative variation % to baseline scenario") %>% 
  kable_styling(latex_options = c("striped","hover","hole_position","repeat_header"))%>% 
  row_spec(0, bold = T)

rm(tablename,Table_Macro_CT)
```
