...
ThreeME combines several features as pointed out @callonnec2013 and [@callonnec2021] :
...
-   ThreeME is a CGEM of neo-Keynesian inspiration. Instead of perfect flexibility hypothesis, prices and quantities slowly adjust, because of uncertainties, adjustment costs or temporal boundaries. Prices do not clear supply and demands. Market imperfections are also included in the model. Consequently, there are disequilibrium between supply and demand. For instance, involuntary unemployment is possible. ThreeME differs from Walrasian CGEM in its dynamics and its transition to the long run.
...
ThreeME has already been adapted to Mexico by [@landarivera2016] and to Indonesia by \[@frédéricg.2017\] . The adaptation of ThreeME to Tunisia has been founded on consultation with Tunisian experts and other stakeholders. The sectoral disaggregation was validated by Tunisian stakeholders (see Fig.X). 21 sectors and 18 products were chosen. The tax structure is based on the supply and use table (SUT) of the national accounts.
...
The nequired data are economic data from national accounts, in particular from Input-Output Tables (IOTs), physical data from energy balance and detailed tax data by product.

The data were collected from Tunisian institutions, in particular ANME (National Agency for Energy Management), INS (National Institute of statistics), ONE (National Energy Observatory), STEG (Tunisian Company of Electricity and Gas), ITCEQ (Tunisian Institute of Competitiveness and Quantitative Studies), Ministry of Energy, Mines and Energy Transition and the Ministry of Economic Development, Investment and International Cooperation.
...
kable(Table_Macro_ENR, booktabs = T, longtable = T, linesep = "", caption = "Macroeconomic impacts of Carbon tax scenario in % deviation to Baseline") %>% 
kable_styling(latex_options = c("striped","hover","repeat_header"), full_width = T) %>% 
row_spec(0, bold = T)
```
...
ggplot() + 
  geom_line( aes(x = Det[[5]][,"year"],y = S5[,"ems_ci_co2_2"]/Baseline[,"ems_ci_co2_0"]*100 - 100, group = 1, color = "Total reduction")) + 
